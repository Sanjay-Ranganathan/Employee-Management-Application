...
class Employee:
    def __init__(self, id, full_name, age, date_of_birth, salary, department):
        self.id = id
...
class EmployeeManagementApp:
    ...
    def load_employees_from_db(self):
        ...
        self.employees = [Employee(row[0], row[1], row[2], row[3], row[4], row[5]) for row in rows]
    ...
    def update_employee_table(self):
        ...
        # Add employees to table with IDs as tags
        for employee in self.employees:
            item_id = self.employee_tree.insert("", "end", values=(employee.id, employee.full_name, employee.age, employee.department, employee.salary))
            self.employee_tree.item(item_id, tags=(employee.id,))
    ...
    def save_employee(self, full_name, age, date_of_birth, salary, department, window):
        ...
        new_employee = Employee(None, full_name, age, date_of_birth, salary, department)
    ...
    # Get the tags (ID) associated with the selected item
        tags = self.employee_tree.item(selected_item)['tags']

        if not tags:
            messagebox.showerror("Error", "Selected employee does not have an ID.")
            return

    # The first tag is assumed to be the ID
        selected_id = tags[0]
    ...
    # Delete the selected employee from the list
        ...
        if not deleted_employee:
            messagebox.showerror("Error", "Unable to find selected employee.")
            return
